...
/**
 * Seul l'administrateur officiel configuré dans les variables d'environnement Vercel a le rôle Admin
 */
function isAdminEmail(email) {
  ...
  const adminEnv = String(process.env.ADMIN_EMAIL || "").trim().toLowerCase();
  const defaultAdmin = "stanleystawa354@gmail.com";

  return (
    (gmailUser && clean === gmailUser) ||
    (smtpUser && clean === smtpUser) ||
    (adminEnv && clean === adminEnv) ||
    clean === defaultAdmin ||
...
/**
 * Compte testeur spécial autorisé sans limite de création de compte
 */
...
/**
 * Détermine si l'email est exempté des restrictions de création / suppression
 */
...
/**
 * Authentifie la requête (Détecte automatiquement si l'e-mail est administrateur officiel)
 */
async function authenticateRequest(req) {
  ...
  if (key === MASTER_API_KEY || key === LEGACY_MASTER_KEY || key.startsWith("stanleystawa_live_") || key === "stawa_live_9f83a7c4e2b1d680") {
    return {
      ...
      is_admin: true,
      key,
      user: { id: "admin", email: process.env.GMAIL_USER || "stanleystawa354@gmail.com", role: "admin", credits: 999999 }
    };
  }
    ...
    if (user) {
      ...
      if (!isAdm && credits <= 0) {
        return {
          authorized: false,
          is_admin: false,
          reason: "Solde de crédits épuisé (0 crédit restant). Veuillez accomplir des quêtes ou recharger votre compte."
        };
      }
