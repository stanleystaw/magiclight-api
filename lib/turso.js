/**
 * lib/turso.js — Client Turso / libSQL HTTP v2 avec gestion des utilisateurs, OTP, historique des inscriptions et quotas
 */
...
class TursoDB {
  ...
  // --- GESTION DES CODES OTP DE VÉRIFICATION EMAIL ---
  ...
  async verifyOtp(email, otpCode) {
    ...
    const record = rows[0];
    if (record.otp_code !== otpCode) return false;
    ...
    // OTP valide -> suppression du code utilisé
    await this.deleteOtp(cleanEmail);
    return true;
  }
  ...
  async createUser(email, passwordHash, apiKey, credits = 100, role = "user") {
    ...
    // Incrémenter le compteur historique d'inscriptions pour cet e-mail
  ...
  async deleteUserAccount(apiKey) {
    ...
    const res = await this.execute(sql, [apiKey.trim()]);
    return true;
  }
