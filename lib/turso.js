...
class TursoDB {
  ...
  // --- GESTION DU SYSTÈME DE QUÊTES & PARRAINAGE ---
  ...
  async claimQuest(email, questId, rewardCredits, dateStr = null) {
    const cleanEmail = String(email || "").trim().toLowerCase();
    const curDate = dateStr || new Date().toISOString().split("T")[0];
    const sql = `
      INSERT INTO user_quests (user_email, quest_id, reward_credits, claimed_date)
      VALUES (?, ?, ?, ?);
    `;
    await this.execute(sql, [cleanEmail, questId, rewardCredits, curDate]);
  ...
  async recordReferral(referrerEmail, newEmail, referrerReward = 30) {
    ...
    const actualReward = isDev ? 15 : 30; // 30 crédits pour Studio, 15 pour Dev
