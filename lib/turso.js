/**
 * lib/turso.js — Client Turso / libSQL HTTP v2 avec Sécurité Multi-Couches & Quotas Stricts
 */
...
class TursoDB {
  ...
  // --- GESTION SÉCURISÉE DES CODES OTP (AVEC COMPTEUR DE TENTATIVES & AUTO-RÉVOCATION) ---
  ...
  async verifyOtp(email, otpCode) {
    ...
    if (record.otp_code !== otpCode) {
      ...
      if (nextAttempts >= 3) {
        await this.deleteOtp(cleanEmail);
        return { valid: false, reason: "3 tentatives incorrectes consécutives. Le code a été révoqué par sécurité. Veuillez redemander un nouveau code." };
      }
      await this.execute(`UPDATE email_verifications SET attempts = ? WHERE email = ?;`, [nextAttempts, cleanEmail]);
      return { valid: false, reason: `Code incorrect (tentative ${nextAttempts}/3). Vérifiez votre boîte Gmail.` };
    }

    // OTP valide -> révocation immédiate du code à usage unique
    await this.deleteOtp(cleanEmail);
    return { valid: true };
  }
  ...
  // --- GESTION DES UTILISATEURS (INSCRIPTION, AUTH & SUPPRESSION) ---
  ...
  async createUser(email, passwordHash, apiKey, credits = 100, role = "user") {
    ...
    // Incrémenter le compteur d'inscriptions pour cet e-mail
