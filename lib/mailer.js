...
async function sendOtpEmail(toEmail, otpCode) {
  ...
  if (process.env.RESEND_API_KEY) {
    try {
      ...
      console.warn("[Resend Error Body]", resendErr);
    } catch (err) {
  ...
  if (transporter) {
    try {
      ...
      return { sent: false, error: `Erreur d'envoi SMTP (${err.message}). Vérifiez votre mot de passe d'application Google sur Vercel.` };
    }
  }

  if (isTester) {
    console.log(`[Tester Note] Compte testeur ${cleanEmail} -> Code ${otpCode} auto-enregistré`);
    return { sent: true };
  }

  console.warn(`[Mailer Warning] Aucun compte Gmail ou SMTP configuré pour envoyer l'e-mail à ${cleanEmail}.`);
  return {
    sent: false,
    error: "Service d'envoi d'e-mail non configuré sur Vercel. Veuillez ajouter GMAIL_USER et GMAIL_PASS (Mot de passe d'application Google) dans vos variables d'environnement Vercel."
  };
}
...
function getEmailHtml(otpCode) {
  return `
    <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;max-width:480px;margin:0 auto;background:#090b10;color:#f0f3f8;border-radius:16px;overflow:hidden;border:1px solid rgba(255,255,255,0.1);padding:36px 28px;text-align:center;">
      ...
      <p style="font-size:13px;color:#8b949e;margin:0 0 28px;">Vérification de votre compte (+30 Crédits offerts)</p>
      ...
      <div style="font-size:11px;color:#545d68;border-top:1px solid rgba(255,255,255,0.08);padding-top:20px;">
        ...
        © 2026 Stanley Stawa AI • Système de Protection Sécurisé
      </div>
    </div>
  `;
}
